...
### Get seed postcodes
old_domain_df = pd.read_csv(final_dir + '/'+ last_dateid+'_domain.csv')
old_domain_df['DateID'] = pd.to_datetime(old_domain_df['DateID'],format='%Y-%m-%d')


##  Create final area_counts

old_domain_df['bedrooms'] = old_domain_df['bed'].apply(lambda x: '5-any' if x > 4 else str(x) )
old_domain_df['bedrooms'].value_counts()

updatefile = output_directory + '01a Region href property/Domain_Back_R4_'+dateid + '.csv'
...
for row in rows4:           # row = rows1[0]
	...
	print(region+ '-bedrooms_' + bedrooms)
	if pd.isnull(area_counts['complete'].iloc[row]):
		#area_counts.iloc[row,:]
		#
		## STEP 1 IDENTIFY URL
		...
		if len(find_max) == 0:
			...
			max_page = pd.Series(find_max).str.split('[^0-9]',expand=True).max().max()
			max_page = int(max_page)/20.0
			max_page = 50 if max_page > 50 else int(np.ceil(max_page))
		#  
		## STEP 4:  GET PAGES
		for page_no in list(reversed(range(1,max_page + 1))):       # page_no=list(reversed(range(1,max_page + 1)))[0]

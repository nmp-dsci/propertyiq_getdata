...
### Get seed postcodes
old_domain_df = pd.read_csv(final_dir + '/'+ last_dateid+'_realestate_cut.csv')
old_domain_df['DateID'] = old_domain_df['HTML__sale_date'].str.extract('.* (\d{2} \w{3} \d{4})',expand=False)
old_domain_df['DateID'] = pd.to_datetime(old_domain_df['DateID'],format='%d %b %Y')

old_domain_df['bedrooms'] = old_domain_df['HTML__beds'].fillna(0).apply(lambda x: '5' if x > 4 else str(int(x)) )

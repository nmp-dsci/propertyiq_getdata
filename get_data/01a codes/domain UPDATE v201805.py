...
updatefile = output_directory + '01a Region href property/Domain_Back_F_'+dateid + '.csv'
...
scrape_area_dir =  output_directory + '01a Region href property/' + dateid +'_domain'
if os.path.exists(scrape_area_dir) == False :
    os.mkdir(scrape_area_dir)

### Get seed postcodes
old_domain_df = pd.read_csv(final_dir + '/'+ last_dateid+'_domain_cut.csv')
old_domain_df['dateID']=old_domain_df['tags__tagText'].str.extract('.* (\d{2} [a-zA-Z]{3} \d{4})',expand=False)
old_domain_df['dateID'] = pd.to_datetime(old_domain_df['dateID'],format='%d %b %Y')

##  Create final area_counts
old_domain_df = old_domain_df.query('features__beds==features__beds')
old_domain_df['bedrooms'] = old_domain_df['features__beds'].apply(lambda x: '5-any' if x > 4 else str(x) )
...
if os.path.exists(updatefile):
    ...
    poa_sub_dateID = old_domain_df.groupby(['address__postcode','address__suburb'])['dateID'].max().reset_index()
    poa_sub_dateID = poa_sub_dateID.rename(columns={'address__postcode':'postcode'
                                                ,   'address__suburb':'suburb'})
    poa_sub_dateID['suburb'] = poa_sub_dateID['suburb'].str.lower().str.replace(' ','-')
...
for row in range(rows):           # row = rows1[0] row = 3843

...
#####################
## Get DATA
auhouse_df = pd.read_csv(master_dir  + '/'+dateid+'_auhouse.csv')
domain_df = pd.read_csv(master_dir  + '/'+dateid+'_domain.csv')
REA_df = pd.read_csv(master_dir  + '/'+dateid+'_REA.csv')
...
auhouse_df = auhouse_df.drop_duplicates('auhouseid',keep='first')
...
fulljoin_df['sale_price'] =fulljoin_df['sale_price'].astype(float)
fulljoin_df.query('YYYYQ>="20120" and postcode=="2061"').groupby('YYYYQ')['sale_price'].mean().plot(kind='line')
fulljoin_df.query('YYYYQ>="20120" and postcode=="2060" and beds == 3 and propertyType=="house"').groupby('YYYYQ')['sale_price'].mean().plot(kind='line')
fulljoin_df.query('YYYYQ>="20120" and postcode=="2095"').groupby('YYYYQ')['sale_price'].mean().plot(kind='line')


fulljoin_df.query('YYYYQ>="20120" and postcode=="2061"').groupby('YYYYMM').size().plot(kind='line')
fulljoin_df.query('YYYYQ>="20120" and postcode=="2060"').groupby('YYYYMM').size().plot(kind='bar')
fulljoin_df.query('YYYYQ>="20120" and postcode=="2258"').groupby('YYYYMM').size().plot(kind='bar',figsize=(12,6))
fulljoin_df.query('YYYYQ>="20120" and postcode=="2095"').groupby('YYYYMM').size().plot(kind='bar',figsize=(12,6))



fulljoin_df.loc[28]['nswgovid']
fulljoin_df.loc[28]['domainid']



nswgov_df.query('nswgovid == 3172945')
domain_df.query('domainid == 2006926557')

...
## Get data
dateid =  pd.Series(os.listdir(output_directory + '01b Suburb_Files')).str.extract('(\d{8})_').max()
print (dateid)
...
print(master_df['REAid'].value_counts().value_counts().describe())
...
# 7.6m
master_df.to_csv(final_dir  + '/'+dateid+'_realestate_all.csv',index=False)
# master_df = pd.read_csv(final_dir  + '/'+dateid+'_realestate_all.csv')
...
###
master_df['rn'] = master_df.groupby(['REAid','column']).cumcount()
master_df = master_df.query('rn==0')

# 84.7k
master_df = master_df.pivot(index='REAid',columns='column', values='value').reset_index()


#####################
## TO CSV
master_df.to_csv(final_dir  + '/'+dateid+'_realestate_cut.csv',index=False)

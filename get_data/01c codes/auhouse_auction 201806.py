...
## Extract DATE
master_df['dateID'] = master_df['url'].str.extract('(\d{4}-\d{2}-\d{2})')
#master_df['dateID'].str.extract('(.*) \d{4}-\d{2}-\d{2}').value_counts()

master_df['state'] = master_df['postcode'].str.extract('(.*)\d{4}').str.strip()
master_df['postcode'] = master_df['postcode'].str.extract('(\d{4})').str.strip()
...
missing_suburb = {  'bilgola plateau':'bilgola'
                  }

...
updatefile = output_directory + '01a Region href property/Domain_Back_F3_'+dateid + '.csv'
if os.path.exists(updatefile):
	area_counts = pd.read_csv(updatefile)
else:
	area_counts = old_domain_df.groupby(['suburb','postcode','bedrooms'])['DateID'].max().reset_index()

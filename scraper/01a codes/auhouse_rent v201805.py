...
### Get seed postcodes
old_auhouse_df = pd.read_csv(final_dir + '/'+ last_dateid+'_'+sourceid+'.csv')
old_auhouse_df['DateID'] = pd.to_datetime(old_auhouse_df['DateID'],format='%Y-%m-%d')

scrape_area_dir =  output_directory + '01a Region href property/' + dateid +'_auhouseprice'
...
if os.path.exists(updatefile):
    ...
    poa_sub_dateID = old_auhouse_df.groupby(['postcode','suburb'])['DateID'].max().reset_index()
...
for row in range(rows):           # row = range(rows)[0] row = 384
